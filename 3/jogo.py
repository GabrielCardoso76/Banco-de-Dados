...
class Nave(pygame.sprite.Sprite):
    def __init__(self, x, y, cor, controles, numero_jogador):
        ...
        self.nivel_tipo_tiro = 0
        self.nivel_resfriamento = 0
        
        # Sistema de Calor
        self.calor_atual = 0
        self.calor_maximo = 100
        self.taxa_aumento_calor_base = 8
        self.taxa_resfriamento_base = 12
    ...
    def atirar(self):
        ...
        if agora - self.ultimo_tiro > self.cadencia_tiro:
            ...
            if self.nivel_tipo_tiro == 0:
                tiro = TiroJogador(self.rect.right, self.rect.centery, self.dano_tiro, self.cor, 0)
...
class Boss(pygame.sprite.Sprite):
    def __init__(self, nivel, jogadores_vivos):
        ...
        self.timers = {'radial': 0, 'mira': 0, 'teleguiado': 0, 'aliado': 0}
        self.cadencias = {'radial': max(1000, 3000 - nivel * 200), 'mira': max(500, 2000 - nivel * 150),
                          'teleguiado': max(2000, 6000 - nivel * 300)}
    ...
    def update(self, dt):
        ...
        self.atirar()
        self.invocar_aliados()

    def invocar_aliados(self):
        agora = pygame.time.get_ticks()
    ...
    def atirar(self):
        ...
        if self.jogadores_vivos:
            ...
            if agora - self.timers['teleguiado'] > self.cadencias['teleguiado']:
                self.timers['teleguiado'] = agora
                missil = TiroTeleguiado(self.rect.centerx, self.rect.centery, alvo)
                todos_sprites.add(missil)
                grupo_tiros_inimigos.add(missil)
...
def tela_upgrade(jogadores_vivos, jogadores_mortos):
    custos = {'velocidade': 50, 'dano': 75, 'cadencia': 100, 'reviver': 200, 'tipo_tiro': 500, 'resfriamento': 120}
    ...
    for i, jogador in enumerate(todos_os_jogadores):
        ...
        botoes_por_jogador[jogador.numero_jogador] = {
            ...
            'resfriamento': Botao(x_base - 100, 350, 200, 40, f"Resfriamento (+${custos['resfriamento']})"),
            'tipo_tiro': Botao(x_base - 100, 400, 200, 40, f"Tiro Triplo (${custos['tipo_tiro']})"),
            'cor': Botao(x_base - 100, 450, 200, 40, "Mudar Cor")
        }
    botao_reviver = None
    ...
    while rodando_upgrade:
        for event in pygame.event.get():
            ...
            for j in jogadores_vivos:
                ...
                if botoes['tipo_tiro'].foi_clicado(event) and j.dinheiro >= custos['tipo_tiro'] and j.nivel_tipo_tiro == 0:
                    j.dinheiro -= custos['tipo_tiro']; j.nivel_tipo_tiro = 1; botoes['tipo_tiro'].ativo = False
        ...
        for i, jogador in enumerate(todos_os_jogadores):
            ...
            if status == "VIVO":
                ...
                desenhar_texto(f"Nv. Vel: {jogador.nivel_velocidade} | Nv. Dano: {jogador.nivel_dano} | Nv. Cad: {jogador.nivel_cadencia}", fonte_normal, BRANCO, tela, x_base, 500, center=True)
                desenhar_texto(f"Nv. Resfr: {jogador.nivel_resfriamento} | Tiro: {'Triplo' if jogador.nivel_tipo_tiro > 0 else 'Único'}", fonte_normal, BRANCO, tela, x_base, 525, center=True)
        ...
        if estado_jogo == "JOGANDO":
            if estado_fase == "INIMIGOS":
                ...
                if agora - ultimo_spawn_inimigo > 2000 and len(grupo_inimigos) < 8:
                    ...
                    todos_sprites.add(inimigo_escolhido)
                    grupo_inimigos.add(inimigo_escolhido)
