...
class Game:
    ...
    def loop_jogo(self):
        ...
        controles_p1 = {'cima': pygame.K_w, 'baixo': pygame.K_s, 'esquerda': pygame.K_a, 'direita': pygame.K_d, 'tiro': pygame.K_v, 'cura': pygame.K_b, 'bomba': pygame.K_c, 'bomba_teleguiada': pygame.K_x}
        controles_p2 = {'cima': pygame.K_UP, 'baixo': pygame.K_DOWN, 'esquerda': pygame.K_LEFT, 'direita': pygame.K_RIGHT, 'tiro': pygame.K_l, 'cura': pygame.K_k, 'bomba': pygame.K_o, 'bomba_teleguiada': pygame.K_p}
        ...
        grupos_de_sprites = { 'todos': pygame.sprite.Group(), 'jogadores': pygame.sprite.Group(), 'tiros_jogadores': pygame.sprite.Group(), 'inimigos': pygame.sprite.Group(), 'tiros_inimigos': pygame.sprite.Group(), 'boss': pygame.sprite.Group(), 'bombas': pygame.sprite.Group(), 'explosoes': pygame.sprite.Group(), 'escutos': pygame.sprite.Group() }
        ...
        def preparar_fase(nivel, jogadores_atuais):
            ...
            inimigos_derrotados = 0
            estado_fase = "INIMIGOS"
            inimigos_para_derrotar = 10 + int((nivel - 1) * 2.5)
        ...
        while rodando:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    for jogador in grupos_de_sprites['jogadores']:
                        ...
                        if event.key == jogador.controles['bomba']: jogador.usar_bomba(grupos_de_sprites, teleguiada=False)
                        if event.key == jogador.controles['bomba_teleguiada']: jogador.usar_bomba(grupos_de_sprites, teleguiada=True)
            ...
            if estado_jogo == "JOGANDO":
                ...
                if estado_fase == "INIMIGOS":
                    if pygame.time.get_ticks() - ultimo_spawn_inimigo > max(500, 2000 - nivel_atual*50) and len(grupos_de_sprites['inimigos']) < (8 + nivel_atual):
                        ultimo_spawn_inimigo = pygame.time.get_ticks()
                    ...
                    if inimigos_derrotados >= inimigos_para_derrotar:
                        ...
                        boss = boss_classe(nivel_atual, list(grupos_de_sprites['jogadores']), self)
                ...
                for inimigo, bombas in colisoes_bomba.items():
                    explosao = Explosao(bombas[0].rect.center); grupos_de_sprites['todos'].add(explosao); grupos_de_sprites['explosoes'].add(explosao); inimigos_derrotados += 1
                ...
                colisao_escudo = pygame.sprite.groupcollide(grupos_de_sprites['inimigos'], grupos_de_sprites['escutos'], True, False, pygame.sprite.collide_circle)
                if colisao_escudo:
                    inimigos_derrotados += len(colisao_escudo)
                    for j in grupos_de_sprites['jogadores']: j.dinheiro += 10 * len(colisao_escudo)
            ...
            for jogador in sorted(list(grupos_de_sprites['jogadores']), key=lambda j: j.numero_jogador):
                ...
                desenhar_barra_vida(self.tela, 120, y_base, jogador.vida, jogador.vida_maxima, 200, 15)
                ...
                desenhar_texto(f"B. Guiada[{pygame.key.name(jogador.controles['bomba_teleguiada'])}]: {jogador.bombas_teleguiadas_disponiveis}", pygame.font.Font(None, 24), ROXO, self.tela, 320, y_base + 65)
                y_offset += 100
            
            if estado_fase == "BOSS" and grupos_de_sprites['boss']:
                boss = grupos_de_sprites['boss'].sprites()[0]
                desenhar_barra_vida(self.tela, self.LARGURA_TELA / 2 - 200, 50, boss.vida, boss.vida_maxima, 400, 25)
