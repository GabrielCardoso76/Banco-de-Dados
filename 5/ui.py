...
class Botao:
    ...
    def desenhar(self, superficie):
        if not self.ativo:
            pygame.draw.rect(superficie, (50, 50, 50), self.rect, border_radius=5)
            desenhar_texto(self.texto, self.fonte, (100,100,100), superficie, self.rect.centerx, self.rect.centery, center=True)
            return
...
def desenhar_barra_vida(superficie, x, y, vida_atual, vida_maxima, largura, altura):
    if vida_atual < 0:
        vida_atual = 0
...
def tela_upgrade(game, jogadores_vivos, jogadores_mortos, num_jogadores):
    custos = {'velocidade': 50, 'dano': 75, 'cadencia': 100, 'reviver': 200, 'tipo_tiro': 500, 'resfriamento': 120, 'cura': 60, 'bomba': 80, 'bomba_teleguiada': 150, 'laser': 700, 'hexagono': 600}
    ...
    num_total_jogadores = len(todos_os_jogadores)
    
    largura_coluna = game.LARGURA_TELA / (num_total_jogadores + 1)

    for i, jogador in enumerate(todos_os_jogadores):
        x_base = largura_coluna * (i + 1)
        y_botoes = 200
        botoes_por_jogador[jogador.numero_jogador] = {
            'velocidade': Botao(x_base - 220, y_botoes, 200, 35, f"Velocidade (+${custos['velocidade']})"),
            'dano': Botao(x_base - 220, y_botoes + 45, 200, 35, f"Dano (+${custos['dano']})"),
            'cadencia': Botao(x_base - 220, y_botoes + 90, 200, 35, f"Cadência (+${custos['cadencia']})"),
            'resfriamento': Botao(x_base - 220, y_botoes + 135, 200, 35, f"Resfriamento (+${custos['resfriamento']})"),
            'cura': Botao(x_base - 220, y_botoes + 180, 200, 35, f"Comprar Cura (+${custos['cura']})"),
            'bomba': Botao(x_base - 220, y_botoes + 225, 200, 35, f"Comprar Bomba (+${custos['bomba']})"),
            'bomba_teleguiada': Botao(x_base - 220, y_botoes + 270, 200, 35, f"Bomba Teleguiada (+${custos['bomba_teleguiada']})"),
            'cor': Botao(x_base - 220, y_botoes + 315, 200, 35, "Mudar Cor"),
            'compra_laser': Botao(x_base + 20, y_botoes, 200, 35, f"Comprar Laser (${custos['laser']})"),
            'compra_triplo': Botao(x_base + 20, y_botoes + 45, 200, 35, f"Comprar Tiro Triplo (${custos['tipo_tiro']})"),
            'compra_hexagono': Botao(x_base + 20, y_botoes + 90, 200, 35, f"Comprar Hexágono (${custos['hexagono']})"),
            'equip_normal': Botao(x_base + 20, y_botoes + 180, 200, 35, "Equipar Tiro Padrão"),
            'equip_laser': Botao(x_base + 20, y_botoes + 225, 200, 35, "Equipar Laser"),
            'equip_hexagono': Botao(x_base + 20, y_botoes + 270, 200, 35, "Equipar Hexágono"),
        }
        ...
        for event in pygame.event.get():
            ...
            if botao_reviver and botao_reviver.foi_clicado(event):
                if jogador_vivo.dinheiro >= custos['reviver']:
                    jogador_vivo.dinheiro -= custos['reviver']; jogador_morto.reset_stats()
                    jogadores_vivos.append(jogador_morto); jogadores_mortos.remove(jogador_morto)
            ...
            for j in jogadores_vivos:
                ...
                if botoes['bomba_teleguiada'].foi_clicado(event) and j.dinheiro >= custos['bomba_teleguiada']:
                    j.dinheiro -= custos['bomba_teleguiada']; j.bombas_teleguiadas_disponiveis += 1
                if botoes['cor'].foi_clicado(event):
                    j.cor_atual_index = (j.cor_atual_index + 1) % len(CORES_NAVE)
                    j.cor = CORES_NAVE[j.cor_atual_index]
                    j.atualizar_imagem()
                
                if botoes['compra_laser'].foi_clicado(event) and j.dinheiro >= custos['laser']: j.dinheiro -= custos['laser']; j.possui_laser = True
                if botoes['compra_triplo'].foi_clicado(event) and j.dinheiro >= custos['tipo_tiro']: j.dinheiro -= custos['tipo_tiro']; j.possui_tiro_triplo = True
                if botoes['compra_hexagono'].foi_clicado(event) and j.dinheiro >= custos['hexagono']: j.dinheiro -= custos['hexagono']; j.possui_hexagono = True
                
                if botoes['equip_normal'].foi_clicado(event): j.arma_ativa = 'normal'
                if botoes['equip_laser'].foi_clicado(event) and j.possui_laser: j.arma_ativa = 'laser'
                if botoes['equip_hexagono'].foi_clicado(event) and j.possui_hexagono: j.hexagono_ativo = not j.hexagono_ativo

        game.tela.fill(PRETO)
        desenhar_texto("ARSENAL DO REI", fonte_titulo, BRANCO, game.tela, game.LARGURA_TELA / 2, 50, center=True)
        for i, jogador in enumerate(todos_os_jogadores):
            x_base = largura_coluna * (i + 1)
            ...
            if status == "VIVO":
                ...
                pygame.draw.line(game.tela, BRANCO, (x_base, 170), (x_base, game.ALTURA_TELA - 150), 2)
                
                botoes = botoes_por_jogador[jogador.numero_jogador]
                desenhar_texto("Melhorias", fonte_normal, BRANCO, game.tela, x_base - 120, y_botoes - 30, center=True)
                for key in ['velocidade', 'dano', 'cadencia', 'resfriamento', 'cura', 'bomba', 'bomba_teleguiada', 'cor']:
                    botoes[key].desenhar(game.tela)
                
                desenhar_texto("Hangar", fonte_normal, BRANCO, game.tela, x_base + 120, y_botoes - 30, center=True)
                botoes['compra_laser'].ativo = not jogador.possui_laser
                botoes['compra_triplo'].ativo = not jogador.possui_tiro_triplo
                botoes['compra_hexagono'].ativo = not jogador.possui_hexagono
                botoes['compra_laser'].desenhar(game.tela)
                botoes['compra_triplo'].desenhar(game.tela)
                botoes['compra_hexagono'].desenhar(game.tela)
                
                desenhar_texto(f"Arma Ativa: {jogador.arma_ativa.upper()}", fonte_pequena, CIANO, game.tela, x_base + 120, y_botoes + 155, center=True)
                botoes['equip_normal'].desenhar(game.tela)
                botoes['equip_laser'].ativo = jogador.possui_laser
                botoes['equip_laser'].desenhar(game.tela)
                
                if jogador.possui_hexagono:
                    botoes['equip_hexagono'].texto = "Desativar Hexágono" if jogador.hexagono_ativo else "Ativar Hexágono"
                    botoes['equip_hexagono'].desenhar(game.tela)

        if botao_reviver:
            botao_reviver.desenhar(game.tela)
